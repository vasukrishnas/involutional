...
def existing_classifiers(x,Y):
    ...
    SNN_model.fit(x,Y,epochs=1,verbose=True)
    ...
    decoder = Dense(6, activation='softmax')(decoder)
    SAE_model = Model(inputs=input_layer, outputs=decoder)
    SAE_model.compile(loss = 'categorical_crossentropy', optimizer = keras.optimizers.SGD(lr=.1))
    SAE_model.fit(x,Y,epochs=1,batch_size=32,verbose=True)
    SNN_model.save("Exis_model/SAE")
    # SAE_model.summary()
    ...
    outputs = layers.Dense(6, name="softmax")(layer4)
    bilstm_model = Model(inputs=input_layer, outputs=outputs)
    bilstm_model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy']) 
    bilstm_model.fit(x,Y,epochs=1,batch_size=32,verbose=True)
    ...
    layer8 = Dense(300)(layer7)
    outputs = layers.Dense(6, activation="relu", name="softmax")(layer8)
    CNN_model = Model(inputs=input_layer, outputs=outputs)  
    CNN_model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    CNN_model.fit(x,Y,epochs=1,batch_size=32,verbose=True)
